...
exports.isLoggedIn = (req, res, next) => {
  ...
  if (req.headers.authorization) {
    ...
    try {
      ...
      console.log(req.headers.authorization); // Log the token if invalid
      return res.status(401).send("Invalid Token"); // Return 401 if token is not valid
    }
  } else {
    // Log missing token info
    console.log(req.headers.authorization);
